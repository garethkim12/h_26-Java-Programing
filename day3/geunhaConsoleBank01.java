...
public class geunhaConsoleBank01 {
	...
	private static void menuDeposit() {
		...
		if (accountDao.deposit(accountNo, amount)) {
			...
			System.out.println("입금할 수 없습니다. (계좌번호를 확인해주세요)");
		}
	}
	...
	private static void menuWidthdraw() {
		...
		if (accountDao.withdraw(accountNo, amount, password)) {
			...
			System.out.println("출금할 수 없습니다. (계좌번호, 비밀번호, 잔액을 확인해주세요)");
		}
	}
	...
	private static int getMenu(String[] menuList) {
		...
		int menu = Integer.parseInt(sc.nextLine()); // nextInt()와 nextLine() 혼용으로 인한 버그 방지
		return menu;
	}

	private static void welcomeMessage() {
		System.out.println("Geunha Console Bank에 오신 것을 환영합니다.");
	}
	
	private static void sayGoodbye() {
		System.out.println("Geunha Console Bank를 이용해 주셔서 감사합니다.");
	}
}